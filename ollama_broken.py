...
def get_collection(db_path="rag_db", collection_name="imagerie"):
    chrom    # TROUBLES THORACIQUES - Informations critiques spécifiques
    elif main_pathology == "thoracic":
        # Signes de gravité thoracique
        thoracic_signs = any(word in text for word in [
            'embolie', 'pneumothorax', 'infarctus', 'dissection', 'aortique',
            'dyspnée', 'cyanose', 'hémoptysie', 'syncope', 'brutal'
        ])
        ...
        if not has_age:
            return "MANQUANT: âge"
        elif not thoracic_signs and not has_negations:
            return "MANQUANT: signes de gravité thoracique"
        else:
            return "COMPLET"b.PersistentClient(path=db_path)
...
def analyze_missing_info(user_input, model="mistral"):
    ...
    # TROUBLES THORACIQUES - Informations critiques spécifiques
    elif main_pathology == "thoracic":
        # Signes de gravité thoracique
        thoracic_signs = any(word in text for word in [
            'embolie', 'pneumothorax', 'infarctus', 'dissection', 'aortique',
...
def generate_clarifying_questions(missing_info, user_input):
    ...
    is_thoracic = any(word in text for word in ['thorax', 'thoracique', 'poitrine', 'douleur thoracique', 'oppression', 'dyspnée'])
    ...
    # TROUBLES THORACIQUES - Questions spécifiques
    elif 'signes de gravité thoracique' in missing_info.lower():
        return "Y a-t-il dyspnée, douleur brutale, syncope, hémoptysie ou signes d'embolie pulmonaire ?"
    ...
    # Questions génériques selon pathologie
        ...
        return "Y a-t-il des signes de gravité thoracique (dyspnée, syncope, douleur brutale) ?"
    else:
...
def generate_imaging_recommendation(user_input, guidelines):
    ...
    elif any(word in text for word in ['thorax', 'thoracique', 'poitrine', 'douleur thoracique', 'oppression', 'dyspnée']):
        # Signes de gravité thoracique
        has_pe_signs = any(word in text for word in ['embolie', 'dyspnée', 'hémoptysie'])
        has_cardiac_signs = any(word in text for word in ['infarctus', 'syncope', 'brutal'])
        has_pneumo_signs = any(word in text for word in ['pneumothorax', 'essoufflement brutal'])
        
        if has_pe_signs:
            return "IMAGERIE INDIQUÉE : Angioscanner pulmonaire - Suspicion embolie pulmonaire"
        elif has_cardiac_signs:
            return "IMAGERIE INDIQUÉE : Scanner thoracique - Douleur thoracique avec signes de gravité"
        elif has_pneumo_signs:
            return "IMAGERIE INDIQUÉE : Radiographie thoracique - Suspicion pneumothorax"
        else:
            return "IMAGERIE INDIQUÉE : Radiographie thoracique - Évaluation douleur thoracique"
